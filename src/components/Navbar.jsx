...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { name: 'Projects', href: '#projects' }, // Added Projects as it's common, though not in original explicitly, good for portfolio
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex-shrink-0">
            <a href="#" className="text-2xl font-bold text-cyan-400 tracking-wider">
              PORTFOLIO
            </a>
          </div>
            ...
            <div className="ml-10 flex items-baseline space-x-4">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-gray-300 hover:text-cyan-400 hover:bg-slate-800 px-3 py-2 rounded-md text-sm font-medium transition-colors duration-300"
                >
                  {link.name}
                </a>
              ))}
